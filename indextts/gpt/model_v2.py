...
class GPT2InferenceModel(GPT2PreTrainedModel, GenerationMixin):
    ...
    def parallelize(self, device_map=None):
        self.device_map = (
            get_device_map(len(self.transformer.h), range(
    ...
    def forward(
        ...
        cache_was_converted = False
        if past_key_values is not None and not isinstance(past_key_values, Cache):
            past_key_values = DynamicCache.from_legacy_cache(past_key_values)
            cache_was_converted = True
        ...
        past_key_values_out = transformer_outputs.past_key_values
        if isinstance(past_key_values_out, Cache):
            past_key_values_out = past_key_values_out.to_legacy_cache()
        elif cache_was_converted:
            past_key_values_out = DynamicCache.from_legacy_cache(past_key_values_out).to_legacy_cache() if past_key_values_out is not None else None
        ...
        if not return_dict:
            other_outputs = transformer_outputs[1:]
            if other_outputs:
                other_outputs = (past_key_values_out,) + other_outputs[1:]
            else:
                other_outputs = (past_key_values_out,)
            return (lm_logits,) + other_outputs
...
class UnifiedVoice(nn.Module):
        ...
        if self.use_accel and torch.cuda.is_available():
            ...
            try:
                ...
                if major < 8 and not (major == 7 and minor >= 5):
                    print(f"Flash Attention requires Turing (7.5)+ or Ampere (8.0)+ GPU. Detected capability {major}.{minor}. Disabling acceleration.")
                    self.use_accel = False
                elif major == 7:
                    print(f"Detected Turing GPU ({major}.{minor}). FlashAttention is experimental on this architecture.")
    ...
    def get_conditioning(self, speech_conditioning_input, cond_mel_lengths=None):
        if self.condition_type == "perceiver":
            if speech_conditioning_input.ndim == 4:
                speech_conditioning_input = speech_conditioning_input.squeeze(
                    1)
            speech_conditioning_input = self.conditioning_encoder(
                speech_conditioning_input)  # (b, d, s)
            conds = self.perceiver_encoder(
                speech_conditioning_input.transpose(1, 2))  # (b, 32, d)
            ...
            if self.condition_type == "conformer_perceiver":
                ...
                conds = self.perceiver_encoder(
                    speech_conditioning_input, conds_mask)  # (b, 32, d)
        elif self.condition_type == "gst":
            if speech_conditioning_input.ndim == 4:
                speech_conditioning_input = speech_conditioning_input.squeeze(
                    1)
            conds = self.gst_encoder(
                speech_conditioning_input.transpose(1, 2))  # (b, 1, d)
        else:
    ...
    def get_emo_conditioning(self, speech_conditioning_input, cond_mel_lengths=None):
        ...
        conds = self.emo_perceiver_encoder(
            speech_conditioning_input, conds_mask)  # (b, 1, d)
    ...
    def forward(self, speech_conditioning_latent, text_inputs, text_lengths, mel_codes, mel_codes_lengths, emo_speech_conditioning_latent,
        ...
        conds = torch.cat((speech_conditioning_latent + emo_vec.unsqueeze(1),
                          duration_emb_half.unsqueeze(1), duration_emb.unsqueeze(1)), 1)
    ...
    def prepare_gpt_inputs(
        ...
        if not single_cond:
            assert conditional_latents.shape[
                0] == b, f"batch size mismatch: {conditional_latents.shape[0]} vs {b}"
        batched_mel_emb = []
        attention_masks = []
        ...
        for i in range(b):
            ...
            text_emb = self.text_embedding(
                text_input) + self.text_pos_embedding.emb(text_input_pos)
            # concatenate [conditional latents][text embeddings]
            conds_text_emb = [
                conditional_latents.squeeze(
                    0) if single_cond else conditional_latents[i],
                text_emb,
            ]
            # +1 for the start_mel_token
            attention_mask = torch.ones(
                target_len+1, dtype=torch.long, device=device)
            # check this text input is padded
            padding: int = L + 2 - text_input.size(-1)
            # pad left of [cond][text] -> [pad][cond][text]
            if padding > 0:
                pad = torch.zeros((padding, conditional_latents.size(-1)),
                                  dtype=text_emb.dtype, device=device)  # [p, dim]
    ...
    def inference_speech(self, speech_condition, text_inputs, emo_speech_condition=None, cond_lengths=None, emo_cond_lengths=None, emo_vec=None, use_speed=False, input_tokens=None, num_return_sequences=1,
        ...
        conds_latent = torch.cat((speech_conditioning_latent + emo_vec.unsqueeze(
            1), duration_emb_half.unsqueeze(1), duration_emb.unsqueeze(1)), 1)
        ...
        if input_tokens is None:
            ...
            attention_mask = F.pad(
                attention_mask, (0, input_tokens.shape[1]), value=1)
        ...
        if typical_sampling:
            ...
            min_tokens_to_keep = 2 if hf_generate_kwargs.get(
                "num_beams", 1) > 1 else 1
            logits_processor.append(TypicalLogitsWarper(
                mass=typical_mass, min_tokens_to_keep=min_tokens_to_keep))
        max_length = (trunc_index + self.max_mel_tokens -
                      1) if max_generate_length is None else trunc_index + max_generate_length
